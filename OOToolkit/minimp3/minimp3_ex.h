...
retry_open:
    ...
    if (file < 0 && (errno == EAGAIN || errno == EINTR))
        goto retry_open;
	int fstatret = fstat(file, &st);
    if (file < 0 || fstatret < 0)
    {
		printf("[minimp3] io error :( %d;%d\n", file, fstatret);
        close(file);
        return MP3D_E_IOERROR;
    }
...
retry_mmap:
	printf("[minimp3] filesize %ld...\n", st.st_size);
    map_info->buffer = (const uint8_t *)mmap(NULL, st.st_size, PROT_READ, MAP_PRIVATE, file, 0);
	if (MAP_FAILED == map_info->buffer && (errno == EAGAIN || errno == EINTR))
	{
		printf("[minimp3] need to retry mmap... %d\n", errno);
		goto retry_mmap;
	}
    close(file);
	if (MAP_FAILED == map_info->buffer)
	{
		printf("[minimp3] mmap fatal failed... %d\n", errno); // <<-- here
		return MP3D_E_IOERROR;
	}
    return 0;
}
...
int mp3dec_load(mp3dec_t *dec, const char *file_name, mp3dec_file_info_t *info, MP3D_PROGRESS_CB progress_cb, void *user_data)
{
    ...
    if ((ret = mp3dec_open_file(file_name, &map_info)))
        return ret;
	printf("[minimp3] open file ret %d\n", ret);
